...
# function `enable` takes a boolean argument and sets self.paused
@external
def enable(_enable: bool):
    self.paused = _enable

@external
@payable
def ping(_dstChainId: uint16, _dstAddress: address, _pings: uint256):
    assert self.paused == False

    # encode the payload with the number of pings
    payload: Bytes[32] = concat(b"",
        convert(_pings, bytes32)
    )

    # use adapterParams v1 to specify more gas for the destination
    version: uint16 = 1
    gasForDestinationLzReceive: uint256 = 350000
    adapterParams: Bytes[CONFIG_SIZE] = concat(
        convert(version, bytes2),
        convert(gasForDestinationLzReceive, bytes32)
    )

    # send LayerZero message
    self._lzSend( # {value: messageFee} will be paid out of this contract!
        _dstChainId, # destination chainId
        payload, # abi.encode()'ed bytes
        self, # (msg.sender will be this contract) refund address (LayerZero will refund any extra gas back to caller of send()
        _dstAddress, # future param, unused for this example
...
def _nonblockingLzReceive(_srcChainId: uint16, _srcAddress: Bytes[40], _nonce: uint64, _payload: Bytes[PAYLOAD_SIZE]):
    sendBackToAddress: address = convert(slice(_srcAddress, 20, 20), address)

    # decode the number of pings sent thus far
    pings: uint256 = _abi_decode(_payload, (uint16, address, uint256))[2]

    # *pong* back to the other side
    payload: Bytes[1000] = _abi_encode(self._chainId, self, pings + 1,  method_id=method_id("ping(uint16,address,uint256)"))
    raw_call(self, payload, value=msg.value)
...
@external
def __init__(_lzEndpoint: ILayerZeroEndpoint, _chainId: uint256):
