...
@internal
def _blockingLzReceive(_srcChainId: uint16, _srcAddress: Bytes[32], _nonce: uint64, _payload: Bytes[1024]):
    pass
...
interface ILayerZeroEndpoint:
    ...
    def receivePayload(srcChainId: uint16, srcAddress: Bytes[32], dstAddress: address, nonce: uint64, gasLimit: uint256, payload: Bytes[1024]): nonpayable
    ...
    def estimateFees(dstChainId: uint16, userApplication: address, payload: Bytes[1024], payInZRO: bool, adapterParam: Bytes[1024]) -> (uint256, uint256): view
    def getChainId() -> uint16: view
    def retryPayload(srcChainId: uint16, srcAddress: Bytes[32], payload: Bytes[1024]): nonpayable
...
interface ILayerZeroMessagingLibrary:
    ...
    def estimateFees(_chainId: uint16, _userApplication: address, _payload: Bytes[1024], _payInZRO: bool, _adapterParam: Bytes[1024]) -> (uint256, uint256): view
...
@external
def _lzReceive(_srcChainId: uint16, _srcAddress: Bytes[32], _nonce: uint64, _payload: Bytes[100]):
...
@internal
def _lzSend(_dstChainId: uint16, _payload: Bytes[1024], _refundAddress: address, _zroPaymentAddress: address, _adapterParams: Bytes[1024], _nativeFee: uint256):
